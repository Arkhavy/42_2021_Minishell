 ...
 * si Buf est NULL, il faut tout de même free l'espace alloué
 * Utilisation : str = getcwd(NULL, 0); avec sécurité malloc
*/
Chdir		:	int		chdir(const char *path);
/**
 * Change Current Working Directory
 * transforme le cwd avec le path donné en param 1
 * Pour fonctionner, le processus doit avoir accès au dossier donné (exécution donc access)
 * retourne 0 if OK, -1 if Error et errno est mis à jour
*/
...
/**
 * L'idéal serait d'utiliser seulement waitpid qui est le plus pratique
 * à la limite wait4, mais les deux autres ne seront probablement pas nécessaire
*/
...
/**
 * Ouvre un dossier de la méme façon qu'un fichier, comme un file descriptor
 * retourne NULL si le dossier n'est pas accessible et errno est mis à jour
 * attention, c'est aussi une allocation mémoire (cf closedir)
*/
readdir		:	struct dirent	*readdir(DIR *dirp);
/**
 * fonctionne comme realloc mais avec opendir et closedir
*/
closedir	:	int		closedir(DIR *dirp);
/**
 * permet de free l'allocation mémoire de opendir
*/
...
/**
 * Ces fonctions sont des routines ??
 * Leur fonctionnement est assez clair dans le man, mais à quoi peuvent-elles servir ?
*/
