...
/*   Updated: 2022/05/19 10:23:24 by ljohnson         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
// {
// 	if (mini_dup_handler(master, fd_link, ))
// }
...
int	mini_child_process(t_master *master, t_cmd *cmd, int last)
{
	...
	if (cmd->token_id == IDT_BTIN)
		mini_built_in_hub(master, cmd, master->fdstruct->fd_link, last);
	else
	{
		pid = fork();
		if (pid == -1)
